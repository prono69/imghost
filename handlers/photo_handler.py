...
async def handle_photo(client: Client, message: Message):
    try:
        # Download the photo
        photo = message.photo  # Get the Photo object
        if photo:
            photo_file_path = await client.download_media(photo.file_id)  # Download the photo by its file_id
            ...
            await message.reply("No photo found in the message.")
    except ConnectError:
